<template>
  <view class="min-h-screen bg-[#0a1128] text-white flex flex-col relative overflow-hidden font-sans">
    ...
    <view class="flex-1 flex flex-col items-center justify-center relative z-10 pb-32">
      ...
      <!-- Text Content -->
      <view class="text-center px-10 z-20">
        <text class="text-5xl font-light tracking-wide block mb-6 text-white/90 animate-fade-in-left" style="animation-delay: 100ms; opacity: 0; animation-fill-mode: forwards;">Avoue!</text>
        <text class="text-gray-400 text-sm font-light leading-relaxed block max-w-[280px] mx-auto animate-fade-in-right" style="animation-delay: 200ms; opacity: 0; animation-fill-mode: forwards;">
          A way to tip people even if you are tight on cash at the moment
        </text>
      </view>

      <!-- Button -->
      <view class="mt-16 animate-fade-in-up z-20" style="animation-delay: 300ms; opacity: 0; animation-fill-mode: forwards;">
        <button 
...
const drawSpiral = (startIndex, endIndex = null) => {
  ...
  const actualEndIndex = endIndex === null ? points.length : endIndex;
  const visiblePoints = points.slice(Math.floor(startIndex), Math.floor(actualEndIndex));
...
const animateSpiralEntrance = () => {
  ...
  const animate = () => {
    ...
    const endIndex = totalPoints * easedProgress;
    drawSpiral(0, endIndex);
